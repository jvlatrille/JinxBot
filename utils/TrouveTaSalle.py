...
class TrouveTaSalle:
    ...
    def refresh(self):
        ...
        print(
            "[Salles] Refresh des emplois du temps...: ",
            self.date.astimezone(timezone).strftime("%d/%m/%Y %H:%M:%S"),
        )
        self.lock = True
        ...
        for salle in tempsalle:
            ...
            while i < len(tempsalle[salle]) - 1:
                ...
                if (
                    tempsalle[salle][i].begin.timestamp()
    ...
    def get_TD_salle(self, TD: list) -> dict:
        ...
        for event in events:
            ...
            if (
                event.begin.date().day > self.date.day
                or event.begin.date().month > self.date.month
                or event.begin.date().year > self.date.year
            ):
                break
    ...
    def get_prof(self, prof: str):
            ...
            for event in self.salles[salle]:
                if event.description != None and prof in event.description:
                    ...
                    if event.name + str(event.begin.timestamp()) in checker:
                        ...
                        checker[event.name + str(event.begin.timestamp())]["salle"] += (
                            "-" + salle
                        )
                    else:
        ...
        if (
            len(prof_info["cours"]) > 0
            and prof_info["cours"][0]["begin"] <= self.date.timestamp()
            and prof_info["cours"][0]["end"] >= self.date.timestamp()
        ):
            prof_info["now"] = prof_info["cours"][0]
            prof_info["cours"].remove(
                prof_info["now"]
            )  # On enleve le cours de la liste des cours
        return prof_info
    ...
    def detecter_creneaux_libres_salle(self, salle: str):
        ...
        if self.salles[salle] == [] and self.date.timestamp() < fin.timestamp():
            ...
            for i in range(len(self.salles[salle])):
                if (
                    i == 0
                    and self.salles[salle][i].begin.timestamp() > self.date.timestamp()
                ):
                    if self.date.hour < 7:
                        creneaux_libres.append(
                            [debut.timestamp(), self.salles[salle][i].begin.timestamp()]
                        )
                    else:
                        ...
                        creneaux_libres.append(
                            [
                                self.date.timestamp(),
                                self.salles[salle][i].begin.timestamp(),
                            ]
                        )

                elif (
                    i != 0
                    and self.salles[salle][i].begin.timestamp()
                ...
                if i == (len(self.salles[salle]) - 1):
                    ...
                    creneaux_libres.append(
                        [self.salles[salle][i].end.timestamp(), fin.timestamp()]
                    )
        ...
        if self.check_salle(salle) == "NOT FOUND":
            ...
            for event in self.salles[salle]:
                if event.end.timestamp() > self.date.timestamp():
                    ...
                    if (
                        event.begin.timestamp() <= self.date.timestamp()
                        and event.end.timestamp() >= self.date.timestamp()
                    ):
                        data["now"] = event_info
                    else:
    ...
    def get_salle_libre(self):
        ...
        for salle in self.salles:
            ...
            if (
                len(creneau) != 0
                and creneau[0][0] <= self.date.timestamp()
                and creneau[0][1] >= self.date.timestamp()
            ):
    ...
    def get_cours_TD(self, anneTDTP: str):
        ...
        for i in range(len(data["cours"])):
            if (
                i < len(data["cours"]) - 1
                and data["cours"][i]["begin"] == data["cours"][i + 1]["begin"]
            ):
                data["cours"][i + 1]["salle"] = (
                    data["cours"][i]["salle"] + "-" + data["cours"][i + 1]["salle"]
                )
                del data["cours"][i]
        return data